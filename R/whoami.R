...
#' username and password, not tokens, and is outside the scope of the \code{yq}
#' package.
#'
#' @inheritParams yq
#'
#' @return A \code{yq_response} object, which is also a \code{list}.
#' @export
#'
#' @examples
#' \dontrun{
...
#' yq_whoami(.token = "8c70fd8419398999c9ac5bacf3192882193cadf2",
...
yq_whoami <- function(.token = NULL, .api_url = NULL, .send_headers = NULL) {
    ...
    res <- yq(endpoint = "/user", .token = .token,
    ...
    ## 'yq_response' class has to be restored
    class(res) <- c("yq_response", "list")
    res
}
